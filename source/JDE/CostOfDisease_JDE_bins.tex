...
\begin{table}[htbp]\centering\captionsetup{width=6.0in}
  ...
  \begin{tabular}{>{\smallsize}l |>{\smallsize}l |>{\smallsize}r |>{\smallsize}c ||>{\smallsize}c}
    ...
    \hline\hline
    Low    & \citet{Brink2025}  &  81,958 & 0.194\% & -9.39 \\
    & & & (50\% of baseline) & \\
    \hline
    Medium & \citet{Gandhi2025} & 132,600 & 0.388\% & -15.47 \\
    & & & (baseline) & \\
    \hline
    High   & \citet{KS2025}     & 192,212 & 0.582\% & -22.48 \\
    & & & (150\% of baseline) & \\
    \hline\hline
  \end{tabular}
  \end{threeparttable}
...
\begin{table}[H] \centering \captionsetup{width=6.0in}
  ...
  \begin{tabular}{lrrr}
    ...
    \hline\hline
    1\% & -6,473.45 & -10,305.77 & -14,623.46 \\
    2\% & -3,337.07 & -5,320.57 & -7,559.16 \\
    3\% & -1,828.62 & -2,921.03 & -4,156.37 \\
    4\% & -1,067.06 & -1,708.40 & -2,435.21 \\
    6\% & -434.00 & -698.72 & -1,000.02 \\
    \hline\hline
  \end{tabular}
\end{table}

These results indicate that the economic burden of disease extends far beyond the direct costs to the health sector. Even in the more optimistic scenario, where the health system absorbs much of the shock, the loss in output is substantial. The high-impact scenario shows that a collapse in treatment access and mitigation capacity could leave GDP on average 3.3\% below the baseline path over the first two decades, with average annual losses of about \$22.5 billion.
...
Applied to South Africa as a primary illustration, our preferred medium scenario implies GDP that is on average about 2.3\% below the baseline path over the first two decades and discounted losses of about \$5.32 trillion at a 2\% discount rate, against an NPV cost of aid in perpetuity of roughly \$25 billion. The implied benefit-to-cost ratio of about 213:1 is illustrative of the potential macroeconomic returns across a broad class of high-burden settings. Using the South Africa GBD HIV mortality profile yields a more data-driven age mapping than a uniform all-age scaling rule, and it also highlights that the timing of output losses can shift materially with the age pattern of mortality.
